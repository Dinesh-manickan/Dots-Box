...
BG = pygame.image.load(Get_Path("assets/Backgound2.png"))
...
def gameloop(running=True):
    mixer.music.stop()
    mixer.music.load('assets/play.mp3')
    mixer.music.play(-1)
    gameover = False
    cells = create_cells()
    ...
    while running:
        ...
        if ccell:
            ...
            if not ccell.winner:
                pygame.draw.circle(
                    SCREEN, RED, (ccell.rect.centerx, ccell.rect.centery), 2)
        ...
        if player == '1':
            pygame.draw.line(SCREEN, BLUE, (p1rect.x, p1rect.bottom+2),
                             (p1rect.right, p1rect.bottom+1), 2)
        else:
            pygame.draw.line(SCREEN, RED, (p2rect.x, p2rect.bottom+2),
                             (p2rect.right, p2rect.bottom+1), 2)
        ...
        if gameover:
            ...
            if p1_score > p2_score:
                winner_img = font.render(f'Player {winner} Won', True, GREEN)
                SCREEN.blit(winner_img, (rect.centerx -
                            winner_img.get_width()/2, rect.centery - 10))
            elif p1_score < p2_score:
                winner_img = font.render(f'Player 2 Won', True, GREEN)
                SCREEN.blit(winner_img, (rect.centerx -
                            winner_img.get_width()/2, rect.centery - 10))
            else:
                winner_img = font.render(f'Player  Tie', True, BLUE)
                SCREEN.blit(winner_img, (rect.centerx -
                            winner_img.get_width()/2, rect.centery - 10))
            ...
            SCREEN.blit(
                msgimg, (rect.centerx-msgimg.get_width()/2, rect.centery + 20))
...
def main_menu():
    ...
    mainsong = mixer.music.load('assets/main2.mp3')
    mixer.music.play(-1)
    ...
    while True:
        ...
        GAME_TITLE = get_font(33).render("Dots and Boxes", True, "#23395D")
        ...
        PLAY_BUTTON = Button(image=None, pos=(width/2, height/2),
                             text_input="PLAY", font=get_font(20), base_color="#3cdfff", hovering_color="White")

        QUIT_BUTTON = Button(image=None, pos=(width/2, height/2 + 50),
                             text_input="QUIT", font=get_font(20), base_color="#8aecff", hovering_color="White")
